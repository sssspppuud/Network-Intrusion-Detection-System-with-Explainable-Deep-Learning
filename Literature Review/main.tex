...
The variety of areas that are vulnerable in these systems highlights the need for a more rigorous and trustworthy cybersecurity system to reduce the number of successful attacks. An effective Intrusion Detection System (IDS) is an essential tool for preventing the wide variety of attacks that IoMT networks face.
...
In \cite{wang2020explainable}, researchers successfully introduced an explainable machine learning framework for IDS. This framework was built around the SHAP method and used the NSL-KDD dataset \cite{5356528} to evaluate the model's performance. However, this paper mentions the framework's limitations and highlights the need for more datasets to be tested to improve the explainability.
...
There is limited work in applying XAI techniques specifically to IoMT scenarios, and the trade-off experienced between interpretabiltity and latency remains under-researched. Aiming to address this gap will allow the development of more trustworthy IDS models in healthcare settings that use IoMT networks.
...
\begin{itemize}
    ...
    \item Integrate XAI techniques into the model (LIME and SHAP), compare the performance of these techniques.
    \item Evaluate the performance of the final model with XAI integrated using a variety of metrics. 
    \item Measure, if any, the latency introduced by integrating XAI and discuss the trade-off between explainability and performance of real-time attack detection.
\end{itemize}
...
The dataset that has been selected for this project does have a large class imbalance, with there being more data in the benign class of network traffic. This could lead to the model showing bias towards non-malicious network traffic, resulting in false negatives in attack detection, putting patient safety at risk. To combat this, the model could employ preprocessing techniques such as SMOTE (Synthetic Minority Over-sampling Technique) during training.
\\
